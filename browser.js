// Klasyczny skrypt przeglądarkowy bez ES modules.
// Wygenerowany z app/app.js; dane zadań ładuje app/tasks-data.browser.js.
...
function parseCsvWords(csvText) {
...
function pickTopWords(words, limit = 10) {
  return [...words]
    .filter((word) => word.lemma && word.translation)
...
function getTaskCount(level = 'A2') {
  return DATA[level]?.length ?? 0;
}

function getTaskSet(level = 'A2', variantIndex = 0) {
...
function generateTextExercise(words = DEFAULT_WORDS, variantIndex = 0, level = 'A2') {
...
function generateRepeatExercise(words = DEFAULT_WORDS, variantIndex = 0, level = 'A2') {
...
function generateTaskSet(words = DEFAULT_WORDS, variantIndex = 0, level = 'A2') {
  return {
...
function placeSelectedWord(currentAnswers, sentenceId, word) {
...
function toggleSelectedWord(currentAnswers, sentenceId, word) {
...
function buildRepeatSummary(exercise) {
